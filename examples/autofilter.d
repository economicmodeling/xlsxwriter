...
int main() {
    ...
    /* Write the row data. */
    foreach (i; 0 .. cast(uint)data.length) {
        worksheet_write_string(worksheet, i + 1, 0, data[i].region.ptr, null);
        worksheet_write_string(worksheet, i + 1, 1, data[i].item.ptr,   null);
        worksheet_write_number(worksheet, i + 1, 2, data[i].volume,     null);
        worksheet_write_string(worksheet, i + 1, 3, data[i].month.ptr,  null);
    }
    /* Add the autofilter. */
